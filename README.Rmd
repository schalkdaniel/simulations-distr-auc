...
dtmp = do.call(rbind, lapply(results, function(ll) do.call(rbind, lapply(ll, function(l) l$aucs))))
...
for (i in seq_along(l2senss)) {
  ...
  ggs[[i]] = ggplot(na.omit(tab), aes(x = "1", y = auc_emp_cut, color = qual, fill = qual)) +
    ...
    ggtitle(latex2exp::TeX(paste0("$\\Delta_2(\\hat{f}) = ", l2senss[i], "$")))
}
...
for (i in seq_along(l2senss)) {
  ...
  ggs_ci[[i]] = ggplot(na.omit(tab), aes(x = "1", y = auc_emp_cut, color = qual_ci, fill = qual_ci)) +
    ...
    scale_fill_npg(labels = unname(latex2exp::TeX(levels(tab$qual_ci)))) + # Looks nicer than uchicago
    #my_color + my_fill +
    labs(fill = "Accuracy") +
    guides(color = "none") +
    facet_grid(paste0("Delta:\n", delta) ~ paste0("Eps.:\n", epsilon)) +
...
gg_ci = ggpubr::ggarrange(ggs_ci[[1]], ggs_ci[[2]], ggs_ci[[3]], ncol = 3, nrow = 1, common.legend = TRUE, legend = "bottom")
